...
__global__ void __launch_bounds__(512, 2) softmax_warp_reduction(const float *cpu_input, float *output, const int rows, const int cols)
{
    int row = blockIdx.x * blockDim.y + threadIdx.y;

    // 避免bank conflict
    __shared__ float input[32*16];
    __shared__ float globalMax[16];
    __shared__ float globalSum[16];

    // copy to shared memory
    input[threadIdx.x + threadIdx.y * blockDim.x] = cpu_input[row * cols + threadIdx.x];

    float max_val = fmaxf(-__FLT_MAX__, input[threadIdx.x + threadIdx.y * blockDim.x]);
...
{
    const size_t rows_per_block = 16;
    ...
    if (!is_aligned128(input)) {
        printf("input is not 128 byte aligned\n");
        return;
    } else {
        printf("input is 128 byte aligned\n");
    }

    if (!is_aligned128(output)) {
        printf("output is not 128 byte aligned\n");
        return;
    } else {
        printf("output is 128 byte aligned\n");
    }
...
int main()
{
    ...
    cudaHostAlloc((void**)&cpu_input, ROWS * COLS * sizeof(float), cudaHostAllocDefault);
    cpu_output = (float *)malloc(ROWS * COLS * sizeof(float));
    // gpu_output = (float *)malloc(ROWS * COLS * sizeof(float));
    cudaHostAlloc((void**)&gpu_output, ROWS * COLS * sizeof(float), cudaHostAllocDefault);
    for (int i = 0; i < COLS * ROWS; i++)
    {
        cpu_input[i] = i % 10;
    }
    softmax_gpu_warp_reduction(cpu_input, gpu_output, ROWS, COLS, 10);
    for (int i = 0; i < 10; i++)
    {
