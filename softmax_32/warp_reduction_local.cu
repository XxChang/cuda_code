...
__global__ void __launch_bounds__(128, 16) softmax_warp_reduction(const float *cpu_input, float *output, const int rows, const int cols)
{
    int row = blockIdx.x * blockDim.y + threadIdx.y;

    float input = cpu_input[row * cols + threadIdx.x];

    float max_val = max(-__FLT_MAX__, input);
...
int main()
{
    ...
    softmax_gpu_warp_reduction(cpu_input, gpu_output, ROWS, COLS, 100);
    for (int i = 0; i < 10; i++)
    {
